...
class TestFriends < MiniTest::Test
  ...
  def setup
#@ allows it to be seen outside the scope
    @person1 = {
      name: "Shaggy",
      age: 12,
      monies: 1,
...
#---
# def test_likes_to_eat_true_false
#   assert_equal
# end
#--------------------------------------
...
#it doesn't have a return in it.
def test_add_new_friend
  initial_length = @person4[:friends].length
  add_new_friend(@person4, "Scrappy-Doo")
  actual = @person4[:friends].length
  assert_equal(initial_length + 1, actual)
end
#---
# def test_add_new_friend
#   add_friend
...
def test_delete_a_friend
  initial_length = @person4[:friends].length
  delete_a_friend(@person4, "Shaggy")
  actual = @person4[:friends].length
  assert_equal(initial_length - 1, actual)
end
...
#array that we can loop over
def test_get_total_money
  expected = 143
  actual = get_total_money(@people)
  assert_equal(expected, actual)
end
